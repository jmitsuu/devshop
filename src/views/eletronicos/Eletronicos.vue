...
const dataEletronics = ref();
const load = ref(false);
async function getMensClouthing() {
  const { data } = await axios.get(
    "https://fakestoreapi.com/products/category/electronics"
  );
  dataEletronics.value = data;
  console.log(data);
  load.value = true;
}
const textHeader = "Os melhores equipamentos para voce! \nDescubra nossa incrível variedade de eletronicos, com equipamentos de alta qualidade. Venha conferir nossa coleção completa!";
...
<template>
  <section class="min-h-screen">
    <header>
      <HeaderGen :title="'Eletronicos'" :text="textHeader" />
    </header>
